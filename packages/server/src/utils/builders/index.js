...
export const getBuildCommand = async (application) => {
    let command = "";
...
export const mechanizeDockerContainer = async (application) => {
    const { appName, env, mounts, cpuLimit, memoryLimit, memoryReservation, cpuReservation, command, args, ports, } = application;
    ...
    const { HealthCheck, RestartPolicy, Placement, Labels, Mode, RollbackConfig, UpdateConfig, Networks, StopGracePeriod, EndpointSpec, Ulimits, } = generateConfigContainer(application);
...
const getImageName = (application) => {
    const { appName, sourceType, dockerImage, registry, buildRegistry } = application;
...
export const getAuthConfig = (application) => {
    const { registry, buildRegistry, username, password, sourceType, registryUrl, } = application;
    if (sourceType === "docker") {
