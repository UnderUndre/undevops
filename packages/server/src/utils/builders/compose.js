...
import { encodeBase64, getEnvironmentVariablesObject, prepareEnvironmentVariables, } from "../docker/utils";
export const getBuildComposeCommand = async (compose) => {
...
export const createCommand = (compose) => {
...
export const getCreateEnvFileCommand = (compose) => {
    const { COMPOSE_PATH } = paths(!!compose.serverId);
...
const getExportEnvCommand = (compose) => {
    if (compose.composeType !== "stack")
        return "";
    const envVars = getEnvironmentVariablesObject(compose.env, compose.environment.project.env, compose.environment.env);
