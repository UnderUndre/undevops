import { getDockerCommand } from "@undevops/server/utils/builders/docker-file";
...
`;
export const getStaticCommand = (application) => {
    const { publishDirectory, isStaticSpa } = application;
    const buildAppDirectory = getBuildAppDirectory(application);
    let command = "";
    if (isStaticSpa) {
        command += getCreateFileCommand(buildAppDirectory, "nginx.conf", nginxSpaConfig);
    }
    command += getCreateFileCommand(buildAppDirectory, ".dockerignore", [".git", ".env", "Dockerfile", ".dockerignore"].join("\n"));
    command += getCreateFileCommand(buildAppDirectory, "Dockerfile", [
        "FROM nginx:alpine",
