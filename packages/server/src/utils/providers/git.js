...
import { findSSHKeyById, updateSSHKeyById, } from "@undevops/server/services/ssh-key";
import { execAsync, execAsyncRemote } from "../process/execAsync";
export const cloneGitRepository = async ({ type = "application", ...entity }) => {
    let command = "set -e;";
    const { appName, customGitUrl, customGitBranch, customGitSSHKeyId, enableSubmodules, serverId, outputPathOverride, } = entity;
...
// 	}
// };
const addHostToKnownHostsCommand = (repositoryURL) => {
    const { SSH_PATH } = paths(true);
    const { domain, port } = sanitizeRepoPathSSH(repositoryURL);
    const knownHostsPath = path.join(SSH_PATH, "known_hosts");
    return `ssh-keyscan -p ${port} ${domain} >> ${knownHostsPath};`;
};
const sanitizeRepoPathSSH = (input) => {
...
export const getGitCommitInfo = async ({ appName, type = "application", serverId, }) => {
