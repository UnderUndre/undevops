...
import { findGiteaById, updateGitea, } from "@undevops/server/services/gitea";
...
const buildGiteaCloneUrl = (giteaUrl, accessToken, owner, repository) => {
...
export const cloneGiteaRepository = async ({ type = "application", ...entity }) => {
    let command = "set -e;";
    const { appName, giteaBranch, giteaId, giteaOwner, giteaRepository, enableSubmodules, serverId, outputPathOverride, } = entity;
    ...
    const cloneUrl = buildGiteaCloneUrl(giteaProvider.giteaInternalUrl || giteaProvider.giteaUrl, giteaProvider.accessToken, giteaOwner, giteaRepository);
    command += `echo "Cloning Repo ${repoClone} to ${outputPath}: ✅";`;
    command += `git clone --branch ${giteaBranch} --depth 1 ${enableSubmodules ? "--recurse-submodules" : ""} ${cloneUrl} ${outputPath} --progress;`;
    return command;
};
export const haveGiteaRequirements = (giteaProvider) => {
    return !!(giteaProvider?.clientId && giteaProvider?.clientSecret);
};
export const testGiteaConnection = async (input) => {
    try {
        const { giteaId } = input;
        if (!giteaId) {
...
export const getGiteaRepositories = async (giteaId) => {
    if (!giteaId) {
        return [];
    }
    await refreshGiteaToken(giteaId);
...
export const getGiteaBranches = async (input) => {
    ...
    const limit = 50; // Max per page
    while (true) {
        const response = await fetch(`${baseUrl}/api/v1/repos/${input.owner}/${input.repo}/branches?page=${page}&limit=${limit}`, {
            headers: {
                Accept: "application/json",
                Authorization: `token ${giteaProvider.accessToken}`,
            },
