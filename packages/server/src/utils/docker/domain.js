...
export const getComposePath = (compose) => {
...
export const loadDockerCompose = async (compose) => {
...
export const loadDockerComposeRemote = async (compose) => {
    const path = getComposePath(compose);
    try {
        if (!compose.serverId) {
            return null;
        }
        const { stdout, stderr } = await execAsyncRemote(compose.serverId, `cat ${path}`);
        if (stderr) {
            return null;
        }
        if (!stdout)
...
export const readComposeFile = async (compose) => {
    const path = getComposePath(compose);
    if (existsSync(path)) {
        const yamlStr = readFileSync(path, "utf8");
        return yamlStr;
...
export const writeDomainsToCompose = async (compose, domains) => {
    try {
...
export const addDomainToCompose = async (compose, domains) => {
    ...
    if (compose.isolatedDeployment) {
        const randomized = randomizeDeployableSpecificationFile(result, compose.isolatedDeploymentsVolume, compose.suffix || compose.appName);
        result = randomized;
    }
    ...
    for (const domain of domains) {
        ...
        if (!result?.services?.[serviceName]) {
            throw new Error(`Domain "${domain.host}" is attached to service "${serviceName}" which does not exist in the compose`);
        }
        ...
        if (!compose.isolatedDeployment) {
            ...
            result.services[serviceName].networks = addDokployNetworkToService(result.services[serviceName].networks);
        }
    }
...
export const writeComposeFile = async (compose, composeSpec) => {
    const path = getComposePath(compose);
    try {
...
export const createDomainLabels = (appName, domain, entrypoint) => {
    const { host, port, customEntrypoint, https, uniqueConfigKey, certificateType, path, customCertResolver, stripPath, internalPath, } = domain;
    ...
    if (middlewares.length > 0) {
        labels.push(`traefik.http.routers.${routerName}.middlewares=${middlewares.join(",")}`);
    }
    ...
    if (entrypoint === "websecure" || (customEntrypoint && https)) {
        if (certificateType === "letsencrypt") {
            ...
            labels.push(`traefik.http.routers.${routerName}.tls.certresolver=${customCertResolver}`);
        }
...
export const addDokployNetworkToService = (networkService) => {
...
export const addDokployNetworkToRoot = (networkRoot) => {
