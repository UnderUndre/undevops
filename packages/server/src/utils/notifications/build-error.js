...
import BuildFailedEmail from "@undevops/server/emails/emails/build-failed";
    ...
    for (const notification of notificationList) {
        ...
        try {
            if (email || resend) {
                ...
                if (resend) {
                    await sendResendNotification(resend, "Build failed for dokploy", template);
                }
            }
            if (discord) {
...
[View Build Details](${buildLink})`,
            ...
            if (pushover) {
                await sendPushoverNotification(pushover, "Build Failed", `Project: ${projectName}\nApplication: ${applicationName}\nType: ${applicationType}\nDate: ${date.toLocaleString()}\nError: ${errorMessage}`);
            }
            if (teams) {
                const limitCharacter = 800;
                const truncatedErrorMessage = errorMessage.substring(0, limitCharacter);
