import { createServiceConfig, loadOrCreateConfig, loadOrCreateConfigRemote, removeTraefikConfig, removeTraefikConfigRemote, writeTraefikConfig, writeTraefikConfigRemote, } from "./application";
import { createPathMiddlewares, removePathMiddlewares } from "./middleware";
export const manageDomain = async (app, domain) => {
    const { appName } = app;
    let config;
    if (app.serverId) {
    ...
    config.http.routers[routerName] = await createRouterConfig(app, domain, domain.customEntrypoint || "web");
...
export const removeDomain = async (application, uniqueKey) => {
...
export const createRouterConfig = async (app, domain, entryPoint) => {
    const { appName, redirects, security } = app;
    const { certificateType } = domain;
    const { host, path, https, uniqueConfigKey, internalPath, stripPath, customEntrypoint, } = domain;
    ...
    if (isRedirectRouter) {
        ...
        if (security.length > 0) {
            ...
            if (domain.domainType === "preview") {
                middlewareName = `auth-${appName.replace(/^preview-(.+)-[^-]+$/, "$1")}`;
            }
            routerConfig.middlewares?.push(middlewareName);
        }
