...
export const addMiddleware = (config, middlewareName) => {
...
export const deleteMiddleware = (config, middlewareName) => {
    if (config.http?.routers) {
        const values = Object.keys(config?.http?.routers);
        for (const routerName of values) {
            const router = config.http.routers[routerName];
            if (router?.middlewares) {
                router.middlewares = router.middlewares.filter((m) => m !== middlewareName);
            }
        }
    }
};
export const deleteAllMiddlewares = async (application) => {
...
export const loadRemoteMiddlewares = async (serverId) => {
    const { DYNAMIC_TRAEFIK_PATH } = paths(true);
    const configPath = join(DYNAMIC_TRAEFIK_PATH, "middlewares.yml");
    try {
        const { stdout, stderr } = await execAsyncRemote(serverId, `cat ${configPath}`);
        if (stderr) {
            console.error(`Error: ${stderr}`);
            throw new Error(`File not found: ${configPath}`);
        }
        const config = parse(stdout);
        return config;
    }
    catch (_) {
        throw new Error(`File not found: ${configPath}`);
...
export const removePathMiddlewares = async (app, uniqueConfigKey) => {
    let config;
    if (app.serverId) {
        try {
            config = await loadRemoteMiddlewares(app.serverId);
