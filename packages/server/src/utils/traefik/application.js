...
export const createTraefikConfig = (appName) => {
...
export const removeTraefikConfig = async (appName, serverId) => {
    try {
...
export const removeTraefikConfigRemote = async (appName, serverId) => {
    try {
        ...
        console.error(`Error removing remote traefik config for ${appName}:`, error);
    }
};
export const loadOrCreateConfig = (appName) => {
    const { DYNAMIC_TRAEFIK_PATH } = paths();
    const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
...
export const loadOrCreateConfigRemote = async (serverId, appName) => {
    const { DYNAMIC_TRAEFIK_PATH } = paths(true);
    const fileConfig = { http: { routers: {}, services: {} } };
    const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
    try {
...
export const readConfig = (appName) => {
    const { DYNAMIC_TRAEFIK_PATH } = paths();
    const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
    if (fs.existsSync(configPath)) {
        const yamlStr = fs.readFileSync(configPath, "utf8");
...
export const readRemoteConfig = async (serverId, appName) => {
    const { DYNAMIC_TRAEFIK_PATH } = paths(true);
    const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
    try {
        const { stdout } = await execAsyncRemote(serverId, `cat ${configPath}`);
...
export const readConfigInPath = async (pathFile, serverId) => {
...
export const writeConfig = (appName, traefikConfig) => {
    try {
        const { DYNAMIC_TRAEFIK_PATH } = paths();
        const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
...
export const writeConfigRemote = async (serverId, appName, traefikConfig) => {
    try {
        const { DYNAMIC_TRAEFIK_PATH } = paths(true);
        const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
        const encoded = encodeBase64(traefikConfig);
        await execAsyncRemote(serverId, `echo "${encoded}" | base64 -d > "${configPath}"`);
    }
    catch (e) {
        console.error("Error saving the YAML config file:", e);
    }
};
export const writeTraefikConfigInPath = async (pathFile, traefikConfig, serverId) => {
    try {
        const configPath = path.join(pathFile);
        if (serverId) {
            const encoded = encodeBase64(traefikConfig);
...
export const writeTraefikConfig = (traefikConfig, appName) => {
    try {
        const { DYNAMIC_TRAEFIK_PATH } = paths();
        const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
...
export const writeTraefikConfigRemote = async (traefikConfig, appName, serverId) => {
    try {
        const { DYNAMIC_TRAEFIK_PATH } = paths(true);
        const configPath = path.join(DYNAMIC_TRAEFIK_PATH, `${appName}.yml`);
        const yamlStr = stringify(traefikConfig);
...
export const createServiceConfig = (appName, domain) => ({
    loadBalancer: {
