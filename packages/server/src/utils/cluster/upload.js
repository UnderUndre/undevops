...
export const uploadImageRemoteCommand = async (application) => {
    ...
    const imageName = application.sourceType === "docker"
    ...
    if (buildRegistry) {
        ...
        if (buildRegistryTag) {
            ...
            commands.push(getRegistryCommands(buildRegistry, imageName, buildRegistryTag));
            commands.push(`echo "⚠️ INFO: After the build is finished, you need to wait a few seconds for the server to download the image and run the container."`);
            commands.push(`echo "📊 Check the Logs tab to see when the container starts running."`);
        }
    }
    if (rollbackRegistry && application.rollbackActive) {
        const deployment = await findAllDeploymentsByApplicationId(application.applicationId);
        ...
        const rollbackRegistryTag = getRegistryTag(rollbackRegistry, rollback?.image || "");
 ...
 */
const extractRepositoryName = (imageName) => {
...
const getRegistryCommands = (registry, imageName, registryTag) => {
    return `
