...
import { IS_CLOUD, paths } from "@undevops/server/constants";
import { getDokployUrl } from "@undevops/server/services/admin";
import { createServerDeployment, updateDeploymentStatus, } from "@undevops/server/services/deployment";
import { findServerById, updateServerById, } from "@undevops/server/services/server";
import { getDefaultMiddlewares, getDefaultServerTraefikConfig, TRAEFIK_HTTP3_PORT, TRAEFIK_PORT, TRAEFIK_SSL_PORT, TRAEFIK_VERSION, } from "@undevops/server/setup/traefik-setup";
...
const installRequirements = async (serverId, onData) => {
    ...
    return new Promise((resolve, reject) => {
        client
            ...
            if (err.level === "client-authentication") {
                ...
                reject(new Error(`Authentication failed: Invalid SSH private key. ${technicalDetail}`));
            }
            else {
