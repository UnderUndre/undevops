...
export const generateRandomDomain = ({ serverIp, projectName, }) => {
...
export const generatePassword = (quantity = 16) => {
 ...
 */
export function generateBase64(bytes = 32) {
    return randomBytes(bytes).toString("base64");
}
function safeBase64(str) {
    return str.replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
}
function objToJWTBase64(obj) {
 ...
 */
export const readTemplateComposeFile = async (id) => {
    ...
    try {
        ...
        const cwd = process.cwd();
        const composeFilePath = join(cwd, ".next", "templates", id, "docker-compose.yml");
