...
export const recordAdvancedStats = async (stats, appName) => {
 ...
 */
export const getHostSystemStats = async () => {
    const osutils = new OSUtils({
        disk: {
...
export const getAdvancedStats = async (appName) => {
    return {
...
export const readStatsFile = async (appName, statType) => {
    try {
        const { MONITORING_PATH } = paths();
...
export const updateStatsFile = async (appName, statType, value) => {
    const { MONITORING_PATH } = paths();
    ...
    await promises.writeFile(`${MONITORING_PATH}/${appName}/${statType}.json`, content);
};
export const readLastValueStatsFile = async (appName, statType) => {
    try {
        const { MONITORING_PATH } = paths();
        const filePath = `${MONITORING_PATH}/${appName}/${statType}.json`;
        const data = await promises.readFile(filePath, "utf-8");
...
export const getLastAdvancedStatsFile = async (appName) => {
    return {
