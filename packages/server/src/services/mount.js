...
import { mounts, } from "@undevops/server/db/schema";
import { createFile, encodeBase64, getCreateFileCommand, } from "@undevops/server/utils/docker/utils";
import { removeFileOrDirectory } from "@undevops/server/utils/filesystem/directory";
import { execAsync, execAsyncRemote, } from "@undevops/server/utils/process/execAsync";
import { TRPCError } from "@trpc/server";
import { eq, sql } from "drizzle-orm";
export const createMount = async (input) => {
    try {
...
export const createFileMount = async (mountId) => {
    try {
        ...
        if (serverId) {
            const command = getCreateFileCommand(baseFilePath, mount.filePath || "", mount.content || "");
...
export const findMountOrganizationId = async (mountId) => {
    const mount = await findMountById(mountId);
...
export const updateMount = async (mountId, mountData) => {
...
export const findMountsByApplicationId = async (serviceId, serviceType) => {
    const sqlChunks = [];
    switch (serviceType) {
        case "application":
...
export const deleteMount = async (mountId) => {
...
export const updateFileMount = async (mountId) => {
    const mount = await findMountById(mountId);
    if (!mount || !mount.filePath)
        return;
...
export const getServerId = async (mount) => {
