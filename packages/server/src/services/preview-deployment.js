...
import { deployments, organization, previewDeployments, } from "@undevops/server/db/schema";
...
export const createPreviewDeployment = async (schema) => {
    ...
    const generateDomain = await generateWildcardDomain(application.previewWildcard || "*.sslip.io", appName, application.server?.ipAddress || "", org?.ownerId || "");
...
const generateWildcardDomain = async (baseDomain, appName, serverIp, _userId) => {
