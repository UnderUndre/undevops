...
import { applications, compose, deployments, environments, projects, } from "@undevops/server/db/schema";
import { removeDirectoryIfExistsContent } from "@undevops/server/utils/filesystem/directory";
...
import { findPreviewDeploymentById, updatePreviewDeployment, } from "./preview-deployment";
...
export async function resolveServicePath(orgId, data) {
    try {
...
export const removeDeployment = async (deploymentId) => {
    try {
        ...
        const message = error instanceof Error ? error.message : "Error removing the deployment";
        throw new TRPCError({
            code: "BAD_REQUEST",
            message,
        });
        ...
        if (serverId) {
            ...
            for (const oldDeployment of deploymentsToDelete) {
                try {
                    ...
                    console.error(`Failed to remove deployment ${oldDeployment.deploymentId} during cleanup:`, err);
                }
            }
            if (command) {
...
async function getApplicationIdsInOrg(orgId, accessedServices) {
    const rows = await db
        ...
        .where(accessedServices !== null
        ? and(eq(projects.organizationId, orgId), inArray(applications.applicationId, accessedServices))
        : eq(projects.organizationId, orgId));
    return rows.map((r) => r.applicationId);
}
async function getComposeIdsInOrg(orgId, accessedServices) {
    const rows = await db
        ...
        .innerJoin(projects, eq(environments.projectId, projects.projectId))
        .where(accessedServices !== null
        ? and(eq(projects.organizationId, orgId), inArray(compose.composeId, accessedServices))
        : eq(projects.organizationId, orgId));
 ...
 */
export const findAllDeploymentsCentralized = async (orgId, accessedServices) => {
...
export const clearOldDeployments = async (appName, serverId) => {
    const { LOGS_PATH } = paths(!!serverId);
