...
import { applications, buildAppName, } from "@undevops/server/db/schema";
import { getAdvancedStats } from "@undevops/server/monitoring/utils";
import { getBuildCommand, mechanizeDockerContainer, } from "@undevops/server/utils/builders";
...
import { ExecError, execAsync, execAsyncRemote, } from "@undevops/server/utils/process/execAsync";
...
import { cloneGitRepository, getGitCommitInfo, } from "@undevops/server/utils/providers/git";
...
import { createDeployment, createDeploymentPreview, updateDeployment, updateDeploymentStatus, } from "./deployment";
import { getDomainHost } from "./domain";
import { createPreviewDeploymentComment, getIssueComment, issueCommentExists, updateIssueComment, } from "./github";
...
export const rebuildPreviewApplication = async ({ applicationId, titleLog = "Rebuild Preview Deployment", descriptionLog = "", previewDeploymentId, }) => {
    const application = await findApplicationById(applicationId);
    const previewDeployment = await findPreviewDeploymentById(previewDeploymentId);
    const deployment = await createDeploymentPreview({
        title: titleLog,
