...
import { patch } from "@undevops/server/db/schema";
import { TRPCError } from "@trpc/server";
import { and, eq } from "drizzle-orm";
import { encodeBase64 } from "../utils/docker/utils";
import { findApplicationById } from "./application";
import { findComposeById } from "./compose";
export const createPatch = async (input) => {
...
export const findPatchById = async (patchId) => {
...
export const findPatchesByEntityId = async (id, type) => {
    return await db.query.patch.findMany({
        where: eq(type === "application" ? patch.applicationId : patch.composeId, id),
        orderBy: (patch, { asc }) => [asc(patch.filePath)],
    });
};
export const findPatchByFilePath = async (filePath, id, type) => {
    return await db.query.patch.findFirst({
        where: and(eq(patch.filePath, filePath), eq(type === "application" ? patch.applicationId : patch.composeId, id)),
    });
};
export const updatePatch = async (patchId, data) => {
...
export const markPatchForDeletion = async (filePath, entityId, entityType) => {
...
export const generateApplyPatchesCommand = async ({ id, type, serverId, }) => {
    const entity = type === "application"
