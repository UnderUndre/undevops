...
import { db } from "../db";
import { deployments as deploymentsSchema, rollbacks, } from "../db/schema";
import { getRegistryTag } from "../utils/cluster/upload";
import { calculateResources, generateBindMounts, generateConfigContainer, generateVolumeMounts, prepareEnvironmentVariables, } from "../utils/docker/utils";
import { execAsync, execAsyncRemote } from "../utils/process/execAsync";
import { getRemoteDocker } from "../utils/servers/remote-docker";
import { findApplicationById } from "./application";
import { findDeploymentById } from "./deployment";
import { safeDockerLoginCommand } from "./registry";
export const createRollback = async (input) => {
    return await db.transaction(async (tx) => {
        ...
        const { deployments: _, bitbucket, github, gitlab, gitea, ...rest } = await findApplicationById(deployment.applicationId);
        await tx
            .update(rollbacks)
            .set({
            image: tagImage,
...
export const findRollbackById = async (rollbackId) => {
...
const deleteRollbackImage = async (image, serverId) => {
...
export const removeRollbackById = async (rollbackId) => {
...
export const rollback = async (rollbackId) => {
    ...
    await rollbackApplication(application.appName, result.image || "", application.serverId, result.fullContext);
};
const dockerLoginForRegistry = async (registry, serverId) => {
    const loginCommand = safeDockerLoginCommand(registry.registryUrl, registry.username, registry.password);
    if (serverId) {
...
const rollbackApplication = async (appName, image, serverId, fullContext) => {
    if (!fullContext) {
    ...
    const { env, mounts, cpuLimit, memoryLimit, memoryReservation, cpuReservation, command, ports, } = fullContext;
    ...
    const { HealthCheck, RestartPolicy, Placement, Labels, Mode, RollbackConfig, UpdateConfig, Networks, Ulimits, } = generateConfigContainer(fullContext);
    const bindsMount = generateBindMounts(mounts);
    const envVariables = prepareEnvironmentVariables(env, fullContext.environment.project.env);
