...
import { getTrustedOrigins, getTrustedProviders, getUserByToken, } from "../services/admin";
...
import { sendEmail, sendVerificationEmail, } from "../verification/send-verification-email";
...
const { handler, api } = betterAuth({
        ...
        user: {
            create: {
                before: async (_user, context) => {
                    if (!IS_CLOUD) {
                        const xDokployToken = context?.request?.headers?.get("x-dokploy-token");
                        if (xDokployToken) {
                            let invitation;
                            try {
                            ...
                            if (_user.email.toLowerCase().trim() !==
                ...
                after: async (user, context) => {
                    ...
                    if (IS_CLOUD) {
                        try {
                            const hutk = getHubSpotUTK(context?.request?.headers?.get("cookie") || undefined);
    ...
    plugins: [
        ...
        ...(IS_CLOUD
            ? [
                admin({
                    adminUserIds: [process.env.USER_ADMIN_ID],
                }),
            ]
            : []),
    ],
...
export const validateRequest = async (request) => {
    ...
    if (apiKey) {
        try {
            ...
            const organizationId = JSON.parse(apiKeyRecord.metadata || "{}").organizationId;
            if (!organizationId) {
                return {
                    session: null,
                    user: null,
            ...
            const member = await db.query.member.findFirst({
                where: and(eq(schema.member.userId, apiKeyRecord.user.id), eq(schema.member.organizationId, organizationId)),
                with: {
                    organization: true,
                },
            });
            // When accessing from DB, use actual column names
            const userFromDb = apiKeyRecord.user;
            const mockSession = {
                session: {
    ...
    if (session?.user) {
        const member = await db.query.member.findFirst({
            where: and(eq(schema.member.userId, session.user.id), ...(session.session.activeOrganizationId
                ? [
