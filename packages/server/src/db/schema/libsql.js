...
import { applicationStatus, EndpointSpecSwarmSchema, HealthCheckSwarmSchema, LabelsSwarmSchema, NetworkSwarmSchema, PlacementSwarmSchema, RestartPolicySwarmSchema, ServiceModeSwarmSchema, sqldNode, UpdateConfigSwarmSchema, } from "./shared";
import { DATABASE_PASSWORD_MESSAGE, DATABASE_PASSWORD_REGEX, generateAppName, } from "./utils";
...
export const apiCreateLibsql = createSchema
    ...
    if (data.sqldNode !== "replica" && data.sqldPrimaryUrl) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["sqldPrimaryUrl"],
            message: "sqldPrimaryUrl should not be provided when sqldNode is not 'replica'.",
        });
    }
});
...
export const apiSaveExternalPortsLibsql = createSchema
    ...
    .superRefine((data, ctx) => {
    if (data.externalPort === null &&
        data.externalGRPCPort === null &&
        data.externalAdminPort === null) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Either externalPort, externalGRPCPort or externalAdminPort must be provided.",
