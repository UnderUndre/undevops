import * as adminService from "@undevops/server/services/admin";
import * as applicationService from "@undevops/server/services/application";
import { deployApplication } from "@undevops/server/services/application";
import * as deploymentService from "@undevops/server/services/deployment";
import * as builders from "@undevops/server/utils/builders";
import * as notifications from "@undevops/server/utils/notifications/build-success";
import * as execProcess from "@undevops/server/utils/process/execAsync";
import * as gitProvider from "@undevops/server/utils/providers/git";
...
vi.mock("@undevops/server/services/application", async () => {
	const actual = await vi.importActual<
		typeof import("@undevops/server/services/application")
	>("@undevops/server/services/application");
	return {
		...actual,
...
vi.mock("@undevops/server/services/admin", () => ({
	getDokployUrl: vi.fn(),
}));

vi.mock("@undevops/server/services/deployment", () => ({
	createDeployment: vi.fn(),
	updateDeploymentStatus: vi.fn(),
	updateDeployment: vi.fn(),
}));

vi.mock("@undevops/server/utils/providers/git", async () => {
	const actual = await vi.importActual<
		typeof import("@undevops/server/utils/providers/git")
	>("@undevops/server/utils/providers/git");
	return {
		...actual,
...
vi.mock("@undevops/server/utils/process/execAsync", () => ({
	execAsync: vi.fn(),
	ExecError: class ExecError extends Error {},
}));

vi.mock("@undevops/server/utils/builders", async () => {
	const actual = await vi.importActual<
		typeof import("@undevops/server/utils/builders")
	>("@undevops/server/utils/builders");
	return {
		...actual,
...
vi.mock("@undevops/server/utils/notifications/build-success", () => ({
	sendBuildSuccessNotifications: vi.fn(),
}));

vi.mock("@undevops/server/utils/notifications/build-error", () => ({
	sendBuildErrorNotifications: vi.fn(),
}));

vi.mock("@undevops/server/services/rollbacks", () => ({
	createRollback: vi.fn(),
}));

import { db } from "@undevops/server/db";
import { cloneGitRepository } from "@undevops/server/utils/providers/git";
