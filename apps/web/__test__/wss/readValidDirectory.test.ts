...
vi.mock("@undevops/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@undevops/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@undevops/server");
