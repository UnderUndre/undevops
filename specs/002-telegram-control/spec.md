...
- Q: Group chats? → **Decision (2026-07-23 review):** write/exec actions **deny-by-default** in groups unless an explicit project ACL allowlists the chat id. Prefer DM for approvals.
- Q: Shared authz with MCP? → **Decision:** single authorization policy module for MCP + Telegram + web; Telegram is transport only.
...
- **FR-012**: The Mini App MUST authenticate via the bound Telegram identity — no separate login flow. The Telegram `initData` MUST be validated per Telegram's documented HMAC scheme before any session is established, resolving identity against the central **Undrlla IdP (`id.undrlla.network`)** per `undrlla/specs/005-sso-jwt-contract.md`.
